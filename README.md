...
`npx nx g @nx/next:app account`

you can check NextJS app using `nx serve account` (you must have nx cli)
...
add to `apps/account/project.json`
...
`npx nx g @nx/next:library ui-components --directory=libs`
...
```json
{
  ...
  "aliases": {
    "components": "@my-org/ui-components/components",
    "utils": "@my-org/ui-components/utils"
  }
}
```
...
import components from library (import `{ DropdownMenu } from '@my-project/ui-components';`)

restart web app (`nx serve account`)
